...
export default function NewNote() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (file.current && file.current.size > config.MAX_ATTACHMENT_SIZE) {
      alert(`Please pick a file smaller than ${config.MAX_ATTACHMENT_SIZE / 1000000
        } MB.`
      );
      return;
    }
  ...
  return (
    <div className="NewNote">
      <Form onSubmit={handleSubmit}>
        ...
        <Stack>
          <LoaderButton size="lg" type="submit" variant="primary" isLoading={isLoading} disabled={!validateForm()}>
            Create
          </LoaderButton>
        </Stack>
      </Form>
